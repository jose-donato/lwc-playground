...
export class MultiTouchChartEvents {
	...
	_addMouseEventListener(
		...
		const boundMouseMoveHandler = (e: MouseEvent) => {
			if (this._mouseState.drawing) {
				e.preventDefault();
			}
			handler.bind(this)(e);
		};
	...
	_addTouchEventListener(
		...
		const boundTouchHandler = (e: TouchEvent) => {
			e.preventDefault();
			handler.bind(this)(e);
		};
		target.addEventListener(eventType, boundTouchHandler);
		const unSubscriber = () => {
			target.removeEventListener(eventType, boundTouchHandler);
		};
		this._unSubscribers.push(unSubscriber);
	}
